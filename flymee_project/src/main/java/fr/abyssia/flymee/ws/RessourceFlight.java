...
public class RessourceFlight {
	...
	public RessourceFlight() {
		...
		Flight flight1 = new Flight(1, "NCE", "ORY", new GregorianCalendar(2021, 4, 4, 17, 30),
				new GregorianCalendar(2021, 4, 4, 19, 00), 4, 2, new Pilot(), userList, new Aircraft(), 81f,
				"Nice");
	...
	public Flight createFlight() {
		Flight newFlight = new Flight(3, "TNS", "ORY", new GregorianCalendar(2021, 6, 4, 17, 30),
				new GregorianCalendar(2021, 6, 4, 19, 00), 4, 2, new Pilot(), new ArrayList<User>(), new Aircraft(), 81f,
				"Tunis");
		...
		for (Flight flight : flightList) {
			if (flight.id == flightID) {
				...
				Flight flightUpdated = new Flight(4, "TNS", "ORY", new GregorianCalendar(2021, 7, 4, 17, 30),
						new GregorianCalendar(2021, 7, 4, 19, 00), 4, 2, new Pilot(), new ArrayList<User>(), new Aircraft(), 81f,
						"Tunis");
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/{id}/departure")
	public List<User> getPassanger(@PathParam("id") int flightID) {
		for (Flight flight : flightList) {
			if (flight.id == flightID) {
